...
def tela_site():
    ...
    st.subheader("Receba alertas de concursos abertos em todo o Brasil")

    st.write(
        "O Radar Concursos Brasil monitora bancas organizadoras e ajuda você "
        "a encontrar oportunidades por estado, cargo e área de interesse."
    )
    ...
    with col2:
        st.metric("Bancas", len(BANCAS))

    with col3:
        st.metric("Plano", "R$ 19,90/mês")

    st.markdown("---")

    st.subheader("O que o sistema oferece")

    st.write("✅ Busca nacional de concursos")
    st.write("✅ Filtros por banca, estado e cargo")
    st.write("✅ Preferências personalizadas")
    st.write("✅ Alertas por e-mail")
    st.write("✅ Acesso ao painel completo")

    st.markdown("---")

    st.subheader("Plano Mensal")
    st.write("💰 **R$ 19,90 por mês**")
    st.write("Ideal para quem quer acompanhar concursos sem perder prazo.")

    if st.button("🔐 Entrar ou cadastrar"):
        st.session_state["mostrar_login"] = True
        st.rerun()


def tela_login():
    st.title("🔐 Acesso ao Radar Concursos Brasil")

    if st.button("⬅️ Voltar para o site"):
...
def tela_principal():
    ...
    if pagina == "Painel inicial":
        ...
        if not estado_pref or not cargo_pref:
            ...
            st.info("Os alertas serão enviados quando o sistema automático for executado.")
